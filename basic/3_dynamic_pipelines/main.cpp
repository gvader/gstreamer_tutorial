...
int main (int argc, char *argv[])
{
    ...
    gst_bin_add_many (GST_BIN (data.pipeline), data.source, data.audio_convert , data.audio_sink, data.video_convert, data.video_sink, NULL);
    if (!gst_element_link (data.audio_convert, data.audio_sink)) {
        g_printerr ("Elements in audio pipeline could not be linked.\n");
        gst_object_unref (data.pipeline);
        return -1;
    }

    if (!gst_element_link (data.video_convert, data.video_sink)) {
        g_printerr ("Elements in video pipeline could not be linked.\n");
        gst_object_unref (data.pipeline);
        return -1;
    }
